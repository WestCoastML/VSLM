...
class DeepSeek(PreTrainedModel):
    ...
    def forward(self, input_ids: torch.Tensor, start_pos: int = 0,labels: torch.Tensor = None,attention_mask: torch.Tensor = None):
        """
        ...
        # RKS - only need last h for inference, but all for trainging
        # h = self.norm(h)[:, -1] 
        h = self.norm(h)
        logits = self.head(h)
        if world_size > 1:
        ...
        loss=None
        if labels is not None:  # Check if labels are provided
            log_probs = torch.log_softmax(logits, dim=-1)  # Apply log softmax
            print(f"{log_probs.shape=} {labels.shape=}")
            gathered_log_probs = torch.gather(log_probs, dim=-1, index=labels.unsqueeze(-1)).squeeze(-1)
            print(f"{gathered_log_probs.shape=}")
            loss = -gathered_log_probs.mean() # Average over all tokens
